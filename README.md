...
![Grad-CAM heatmaps — one correctly-classified test image per class](docs/assets/gradcam_examples.png)

In every panel the model's attention (red) concentrates **on the lesion**, not the surrounding
skin. Note the **Benign Keratosis** example: despite heavy **hair** across the image, the model
focuses on the lesion and ignores the hairs — evidence it learned lesion morphology rather than
dataset artifacts. All seven shown are correctly classified (`[OK]`).
